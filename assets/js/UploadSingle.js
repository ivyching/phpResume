    ...
    initialDropZone(){
        ...
        var deleteBtn=$("#"+self.field).find(self._selector.mainAttachmentList);
            var attachmentDropZone = new Dropzone("#"+self.field+" div.js-attachment-dropzone", {
                url: Routing.generate('upload_multipleFile', {uuid: self.uuid, fieldName: self.field,func:self.func}),
                paramName: 'file',
        ...
        refreshData(){
            var self = this;
            $.ajax({
                url: Routing.generate("refresh_file", {uuid: self.uuid,fieldName:self.field}),
                method: 'POST',
                datatype: 'json',
